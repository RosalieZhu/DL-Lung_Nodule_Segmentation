function [studyID, seriesID] = LIDC_split_annotations_series(xml_path, filename)
...
while ~isempty(reading_sessions.item(c1))
    ...
    if strcmpi(reading_sessions.item(c1).getNodeName, 'ResponseHeader')
        ...
        seriesID = getSeriesUID(reading_sessions.item(c1));
...
function studyID = getSeriesUID(responseHeaderNode)
    ...
    while ~isempty(contentNodes.item(c1))
        ...
        if strcmpi(contentNodes.item(c1).getNodeName, 'SeriesInstanceUid')
